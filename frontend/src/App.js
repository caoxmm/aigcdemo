...
const CONTRACT_ADDRESS = "0xE7874D7C475b084aD8417b0C05F62e8ed068801d"
...
function App() {
        ...
        const connectToMetaMask = async () => {
            ...
            if (typeof window.ethereum !== "undefined") {
                try {
                    ...
                    // Set up a new ethers.js provider with the user's selected network
                    const newProvider = new ethers.providers.Web3Provider(window.ethereum);

                    // Set the provider and signer states
                    setProvider(newProvider);
                    setSigner(newProvider.getSigner());

                    // Set up a new contract instance with the MyNFT contract ABI and address
                    const newMyNFT = new ethers.Contract(CONTRACT_ADDRESS, CONTRACT_ABI, newProvider.getSigner());

                    // Set the MyNFT state
                    setMyNFT(newMyNFT);

                    // Set the user's account address state
    ...
    const handleMintNFT = async () => {
        try {
            // Mint a new NFT with the current user's address
            const tx = await myNFT.mint(address);

            // Wait for the transaction to be confirmed and get the new token ID
            await tx.wait();
            const newTokenId = await myNFT.totalSupply();

            // Update the status with the new token ID
            setStatus(`Minted new NFT with token ID ${newTokenId}`);
            setTokenId("");
        } catch (error) {
            console.error(error);
            setStatus("Error minting new NFT");
        }
    };

    return (
        <div className="App">
            {provider && signer && myNFT && (
                <div>
                    <h1>Mint NFT</h1>
                        <label>
                            Token ID:
